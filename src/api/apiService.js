...
class Api {
  ...
  static post(uri, data, contentType = 'application/json', apiUrl = Config.API_URL) {
    ...
    const requestData = {
      ...
      body: contentType === 'application/json' ? JSON.stringify(decamelizeData) : data
    };
    return Api.getTokenHeader()
      .then((headers) => {
        requestData.headers = { ...requestData.headers, ...headers };
  ...
  static patch(uri, data, contentType = 'application/json', apiUrl = Config.API_URL) {
    const decamelizeData = humps.decamelizeKeys(data);
    const requestData = {
      method: 'patch',
      headers: {
